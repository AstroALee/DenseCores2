...
# -----------
#            0           1        2    3     4     5--
# python newoneDplot.py filename idx units dir [ rows ]
...
if(len(sys.argv)<5):
    ...
    print("python newoneDplot.py fname idx units(y or n) dir(Z or R) [ rows ]")
    exit(1)
...
data = genfromtxt(fname,delimiter=",",unpack=True,skip_header=2)
# Read in contour array
flen = len(data[0,:])
VCont = genfromtxt(fname,delimiter=",",unpack=True,skip_header=1,skip_footer=flen)
# M, N, zL (pc), rRatio (entire box), mExcess (sol), beta, n, Rbdy, Lambda, Pc2Code, Sol2Code
Params = genfromtxt(fname,delimiter=",",unpack=True,skip_footer=flen+1)
...
Cs2Code = 1.0/(18821.9544) # 10 K, 2.33 mu
...
PlotThese = sort(list(set(sys.argv[5:]))) # gets rid of duplicates
...
YCordAll = []
if(idx < 7):   # data from output file (non-derived quantities)
    YCordAll = data[idx,:]
elif(idx==7):
    YCordAll = data[8,:]
elif(idx==8):  # Phi
    ...
    YCordAll = multiply( multiply(data[2,:],data[7,:]) , exp(-data[4,:]) )
    #YCordAll = data[8,:]
elif(idx==10): # computes B = curl(A) = (1/r)*dr(rA)
    Aval = data[5,:]
    rA = multiply(data[2,:],data[5,:])
    rArray = data[2,:]
    curlA = zeros(len(data[2,:]))
    for i in range(0,len(data[2,:])):
        rAval = 0
        rval = 0
        if( rArray[i]==0 ): #on left side
            rAval = 2.0*(Aval[i+N] - Aval[i])/DeltaR
            rval = 1.0
        elif( i/N > 0  and  i/N < N-1 ):  #not on left or right side where there's no r derivatives by symmetry
            rAval = (rA[i+N] - rA[i-N])/2.0/DeltaR
            rval = rArray[i]
        else: # on right side
            rAval = (rA[i] - rA[i-N])/1.0/DeltaR
            rval = rArray[i] - 0.5*DeltaR
        if(rval != 0):
            curlA[i] = rAval/rval
    YCordAll = curlA
...
if(PlotEQ):
    ...
    if(idx==4):
        ...
        Yanal = [ AS.PHIeqn(i,VCont[0],beta,data[8,N-1],Rbdy,nCyl) for i in XCordAnal]
    elif(idx==9):
        Yanal = [ AS.DQDZeqn(i,VCont[0],beta,data[8,N-1],Rbdy,nCyl) for i in XCordAnal]
    elif(idx==10):
        Yanal = [ 0 for i in XCordAnal]
...
if(Units==1):
    ...
    if(idx==4): # V
        plotConvert = pow(1.0/Cs2Code,2)
    elif(idx==5): # A
        plotConvert = 1.0
    elif(idx==6): # Q
        plotConvert = pow(Pc2Code,3)/Sol2Code/pow(Cs2Code,2)
    elif(idx==7):
        plotConvert = pow(Pc2Code,3)/Sol2Code

# undo plotConvert units for now
plotConvert = 1
...
if(PlotEQ):
    XCordAnal = XCordAnal*disConvert
    Yanal = array(Yanal) * plotConvert
...
if(idx==10): ytit = 'curl(A)'

if(Units): ytit = ytit + '  (ndim)'
else: ytit = ytit + '  (ndim)'
plt.title(ytit)
...
if(PlotAS and Slice=='Z'):
    ...
    if(PlotEQ):
        ...
        if(idx==6): plt.plot(AS.R[beta],AS.Q[beta],'--',color=clr,linewidth=linewidth,label=r'Analytic')
